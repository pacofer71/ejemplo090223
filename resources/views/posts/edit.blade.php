<x-app-layout>
    <x-inicio>
        <form name="az" action="{{ route('cposts.update', $cpost) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('put')
            @bind($cpost)
                ...
                <x-form-group inline label="Tags">
                    ...
                    @foreach ($tags as $k => $v)
                        @if (in_array($k, $tagsSelected))
                            <x-form-checkbox name="etiquetas[]" label="{{ $v }}" value="{{ $k }}"
                                :show-errors="false" default />
                        @else
                            <x-form-checkbox name="etiquetas[]" label="{{ $v }}" value="{{ $k }}"
                                :show-errors="false" />
                        @endif
            ...
            <div class="mt-4 flex">
                ...
                <div>
                    <img src="{{ Storage::url($cpost->imagen) }}" class="object-center object-cover" id="img1" />
                </div>
